...
Map<String, String> buildHeaderTokens() {
  ...
  var headerToken = loggedInUser.value.access!.isNotEmpty
      ...
      : '';
  if (isLoggedIn.value) header.putIfAbsent(HttpHeaders.authorizationHeader, () => 'Bearer $headerToken');
...
Future<Response> buildHttpResponse(
  ...
  try {
    ...
    if (isLoggedIn.value && response.statusCode == 401 && !endPoint.startsWith('https')) {
    ...
    if (response.body.isJson()) {
      ...
      if (response.statusCode.isSuccessful()) {
        if (body is Map && body.containsKey('status') && body['status'] is bool && !body['status']) {
          if(body.containsKey('is_registered') && body['is_registered'] is bool && !body['is_registered']) {
            throw body['is_registered'] ?? parseHtmlString(body['message'] ?? errorSomethingWentWrong);
          }else {
            if(body.containsKey('is_registered') && body['is_registered'] is bool && !body['is_registered']) {
              throw body['is_registered'] ?? parseHtmlString(body['message'] ?? errorSomethingWentWrong);
            }else{
...
Future<void> sendMultiPartRequest(MultipartRequest multiPartRequest, {Function(dynamic)? onSuccess, Function(dynamic)? onError}) async {
  try {
    ...
    log(e.toString());
    onError?.call("internetNot Available");
  } on Exception catch (e) {
    log(e.toString());
    onError?.call(errorSomethingWentWrong);
  }
...
Map<String, String> buildHeaderForAppConfiguration() {
  ...
  // if (isLoggedIn.value) {
  //   header.putIfAbsent('user_id', () => userId.toString()); // if user is logged in pass the user id
  // }
